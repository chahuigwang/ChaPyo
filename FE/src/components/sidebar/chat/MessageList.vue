...
<template>
  <div class="flex-1 flex flex-col min-h-0">
    <div ref="scrollEl" class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
      <MessageBubble
        v-for="m in messages"
        :key="m.id"
        :message="m"
      ...
      <!-- 대기 버블: 경과 시간 + 취소 버튼 -->
      <div v-if="isTyping" class="flex gap-1.5 justify-start">
        <div class="h-7 w-7 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300
                    flex items-center justify-center text-sm shrink-0">
          {{ AI_EMOJI }}
        </div>
        <div class="flex flex-col gap-1.5">
          <div class="bg-slate-100 dark:bg-slate-800 rounded-2xl rounded-tl-sm px-3.5 py-2.5 flex items-center gap-2.5">
