...
import { X, MapPin, Phone, Heart, Loader2, UserRound, Check, CheckCircle, Star, ChevronDown, LoaderPinwheel } from 'lucide-vue-next'
...
<template>
  <Transition name="focus-panel">
    <div
      v-if="data"
      class="absolute top-3 right-3 bottom-3 z-20 w-[340px] max-w-[calc(100%-1.5rem)]
             flex flex-col rounded-2xl bg-white/95 dark:bg-slate-900/95 shadow-2xl backdrop-blur-md overflow-hidden"
    >
      <!-- Close -->
      <button
        @click="emit('close')"
      ...
      <div class="flex-1 min-h-0 overflow-y-auto">
        <!-- Image -->
        <div class="relative w-full h-40 shrink-0 bg-slate-100 dark:bg-slate-800">
          <img
...
<style scoped>
.focus-panel-enter-active { transition: opacity 0.25s ease, transform 0.3s cubic-bezier(0.22,1,0.36,1); }
.focus-panel-leave-active { transition: opacity 0.18s ease, transform 0.18s ease; }
.focus-panel-enter-from { opacity: 0; transform: translateX(16px) scale(0.98); }
.focus-panel-leave-to { opacity: 0; transform: translateX(16px) scale(0.98); }
