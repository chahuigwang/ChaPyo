...
export function buildCustomPersona({ gender, ageGroup, mbti }) {
  const id = `custom_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`
  const name = `${gender} · ${ageGroup} · ${mbti}`
  return {
    id,
    name,
    emoji: '👤',
    tagline: `${gender}/${ageGroup}/${mbti} 맞춤 추천`,
    greeting: `${name} 취향으로 추천을 도와드릴게요. 가고 싶은 지역이나 분위기를 알려주세요.`,
    builtIn: false,
    gender,
    ageGroup,
    mbti,
...
export function findPersona(list, id) {
